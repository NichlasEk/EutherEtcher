...
if [ "$(id -u)" -eq 0 ]; then
  install -m 0755 target/release/eutheretcher /usr/local/bin/eutheretcher
  ...
  printf '  sudo install -m 0755 target/release/eutheretcher /usr/local/bin/eutheretcher\n'
